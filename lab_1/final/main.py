...
def run_all() -> None:
    ...
    print("\n" + "=" * 82)
    # print(f"{'Algorithm':<14} {'Time(ms)':>9} {'Explored':>10} "
    #       f"{'PathCost':>12}  f(n) formula")
    # print(f"{'Algorithm':<14} {'Time(ms)':>9} " f"{'PathCost':>12}  f(n) formula")
    print(f"{'Algorithm':<14} {'Time(ms)':>9} {'PathCost':>12}")
    print("=" * 82)
    for r in results:
        n = r["name"]
        # print(f"{n:<14} {r['time']:>9.1f} {r['nodes']:>10} "
        #       f"{r['cost']:>12.1f}  {FN_NOTES.get(n, '')}")
        print(f"{n:<14} {r['time']:>9.1f}  "
              f"{r['cost']:>12.1f} ")
    print("=" * 82)
