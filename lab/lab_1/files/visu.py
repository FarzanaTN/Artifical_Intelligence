...
#     """
#     Three-panel complexity comparison — all correctly labelled:

#       Panel 1 — Wall-clock time (ms, log scale)
#         Measures actual runtime including heuristic computation overhead.

#       Panel 2 — Nodes explored (log scale)  ← TIME proxy
#         Every algorithm counts the same event: a node is popped from the
#         frontier and processed.  This is proportional to the number of
#         edge relaxations, making it a fair cross-algorithm time comparison
#         independent of Python overhead.

#       Panel 3 — Max frontier size (log scale)  ← SPACE proxy
#         The largest number of nodes held in memory simultaneously.
#         BFS/A* = max open-list size.  DFS/DLS/IDA* = max stack/path depth.
#         This is the real space complexity measure — NOT nodes_explored.
#     """
#     names  = [r["name"]  for r in results]
#     colors = [ALGO_COLORS.get(n, PALETTE["accent"]) for n in names]

#     fig, axes = plt.subplots(1, 3, figsize=(28, 7), facecolor=PALETTE["bg"])
#     _bar_chart(axes[0], names, [r["time"]         for r in results],
#                colors, "Wall-clock Time",
#                "ms (log scale)")
#     _bar_chart(axes[1], names, [r["nodes"]        for r in results],
#                colors, "Time Complexity  (nodes explored)",
#                "Nodes popped from frontier (log scale)")
#     _bar_chart(axes[2], names, [r["max_frontier"] for r in results],
#                colors, "Space Complexity  (peak frontier size)",
#                "Max nodes in memory at once (log scale)")
...
def plot_complexities(results: list):
    ...
    # _save(fig, "time_complexity.png")


    # -------- Nodes Explored (Time Complexity Proxy) --------
    fig, ax = plt.subplots(figsize=(12,7), facecolor=PALETTE["bg"])
    _bar_chart(
        ax,
        names,
        [r["nodes"] for r in results],
        colors,
        "Time Complexity (Nodes Explored)",
        "Nodes popped from frontier (log scale)"
    )
    fig.tight_layout(pad=2)
    _save(fig, "nodes_explored_complexity.png")


    # -------- Max Frontier (Space Complexity) --------
    fig, ax = plt.subplots(figsize=(12,7), facecolor=PALETTE["bg"])
    _bar_chart(
        ax,
        names,
        [r["max_frontier"] for r in results],
        colors,
        "Space Complexity (Max Frontier Size)",
        "Max nodes in memory at once (log scale)"
    )
    fig.tight_layout(pad=2)
    _save(fig, "space_complexity.png")
